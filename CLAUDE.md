...
- `reward_consistency()` returns +1.0 if the model's answer TEXT (option content, not index) matches any in-batch sibling from the same BBQ template family. Compares text not index because demographic variants permute the (a)/(b)/(c) ordering.
...
- `make_reward_fn()` wraps `compute_reward` into TRL's GRPOTrainer signature (takes `completions`, `**kwargs` with `prompts` and dataset columns forwarded via `remove_unused_columns=False`)
...
- `make_reward_fn()` reads `template_family_key` from batch kwargs to identify siblings within each reward batch — no separate `family_map` parameter needed
...
- **Primary bias metric**: Official BBQ bias score matching `BBQ_calculate_bias_score.R` exactly, implemented in `compute_bias_score()`:
...
- **Group fairness metrics**: DPD, EOD, DIR, RB from Ravulu et al. 2024 — computed in `compute_group_fairness_metrics()`
...
- `FairRLVRCallback`: logs reward mean/std per step via `on_step_end`; saves CoT samples at configurable checkpoint steps (proportionally spaced at 3%, 14%, 29%, 43%, 57%, 71%, 86%, 100% of `num_train_steps`)
...
- W&B metrics are sent automatically when `wandb.run is not None` (no separate `use_wandb` flag needed in callbacks)
...
- **Training uses bfloat16** (no quantization); evaluation also loads in bfloat16. 4-bit is disabled by default — the H100 has sufficient VRAM for full bf16 training at batch_size=32.
...
- **Group size G=4**: Default in all configs. For 3-choice MCQA the reward signal is well-estimated at G=4; if entropy collapses, raise G. G=2 can be used for dry-run mode only.
...
- **Sibling co-batching**: When `alpha_consistency > 0`, `create_splits(sort_by_family=True)` sorts by `question_index` so demographic variants of the same BBQ template land in adjacent positions. `make_reward_fn()` groups prompts by `template_family_key` within each batch to find siblings.
- The `ground_truth_map` keyed on formatted prompt strings is the mechanism tying GRPO completions back to their ground-truth labels — rebuilt from scratch each run. A warning is printed if any prompt has no ground truth.
...
| Quantization | None (bfloat16, training); bfloat16 (eval) |
| LoRA r / α | 16 / 32 |
| Group size G | 4 |
| Batch size | 32 per device (grad_accum=1 → effective batch=32) |
| Max new tokens | 256 |
| Learning rate | 1e-5 |
| KL coefficient | 0.01 |
...
- **Exp 1 (Main)**: Zero-shot vs SFT vs GRPO λ=0 vs Fair-RLVR λ=0.5. Reports BBQ accuracy (ambig/disambig), official bias score (primary), secondary bias score, group fairness metrics, MMLU, GSM8K.
...
| Batch accuracy | FairRLVRCallback | `train/accuracy` |
| R_fairness avg per batch | FairRLVRCallback | `train/r_fairness` |
| R_consistency avg per batch | FairRLVRCallback | `train/r_consistency` |
| P_structural avg per batch | FairRLVRCallback | `train/p_structural` |
| Stereotype pick rate (ambig) | FairRLVRCallback | `train/stereotype_rate_ambig` |
| Abstention rate | FairRLVRCallback | `train/abstention_rate` |
| Sibling hit rate | FairRLVRCallback | `train/sibling_hit_rate` |
...
- `logs/batch_logs.json` — per-batch reward breakdown (r_fairness, r_consistency, p_structural)
