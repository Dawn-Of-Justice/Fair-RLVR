...
Fair-RLVR ($\lambda=0.5$) improves ambiguous-context accuracy from 66.1\% to 86.7\% and reduces ASE from 24.3\% to 8.4\%, while SFT's bias score worsens (0.723 vs.\ 0.717 zero-shot), isolating the fairness reward as the debiasing mechanism. Causal faithfulness analysis ($\Delta_{\text{null}} = 0.22$) confirms the chain-of-thought plays a genuine computational role. OOD evaluation confirms transfer: GRPO leads on WinoBias and StereoSet language quality, while Fair-RLVR achieves the highest intersectional BBQ ambiguous accuracy (96.2\%).
