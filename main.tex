...
FairReason \cite{fairreason} is the closest work to ours, using GRPO on a 1:4 bias-to-reasoning data ratio to reduce stereotype scores by 10\% while retaining 88\% of reasoning accuracy. It differs in three ways: it targets multimodal models, uses BBQ only for evaluation (not training), and does not analyze causal faithfulness. C2BMs \cite{c2bm} provide theoretical grounding by showing that causal reasoning bottlenecks improve robustness and enable ground-truth fairness interventions. Counterfactual data augmentation \cite{cda} introduced the idea of generating demographic variants of the same sentence to improve fairness; our $R_{\text{consistency}}$ reward adapts this principle to the RL setting by rewarding answer invariance across in-batch demographic siblings.

\noindent\textbf{Positioning.} Unlike all prior work, Fair-RLVR uses a bias benchmark exclusively as a \emph{training} reward signal (not evaluation), applies RLVR to a text-only setting with a verifiable ground-truth label for each example, and provides interventional causal evidence that the learned chain-of-thought is computationally necessary.
...
SFT achieves the best DPD (0.072), DIR (0.907), and RB (0.264), while GRPO achieves the best EOD (0.167). Fair-RLVR is intermediate on all four metrics. Importantly, SFT's strong group-fairness scores are largely a function of its near-perfect accuracy (97.8\%): when almost no errors are made, demographic error-rate differences are small by construction, regardless of whether the remaining errors are stereotype-aligned. This is why SFT's group fairness metrics appear favourable even though its stereotype error rate (0.723) is \textit{worse} than zero-shot. GRPO ($\lambda=0$) shows the worst DPD (0.247) and DIR (0.673), consistent with its low ambiguous-context accuracy and high abstention rate creating uneven error distributions across groups. Fair-RLVR's intermediate position on group fairness reflects a genuine trade-off: it reduces stereotype-consistent errors (low bias score) at the cost of elevated abstention, which introduces some asymmetry in group-level error rates. The EOD value of 0.188 reflects remaining variation in true positive rates across social categories, driven primarily by Age and Physical Appearance (Table~\ref{tab:percategory}).
...
The SES worsening is largely an artifact of accuracy improvement: total SES errors fell from 136 (zero-shot) to 50 (Fair-RLVR), but the 50 remaining errors are highly stereotype-concentrated (47/50 = 0.940). The model learned to answer most SES questions correctly, but the residual hard cases cluster around SES stereotypes that are also common in the training data (Nationality has a similar pattern: 340 ambiguous questions, high pre-training prior for occupational nationality stereotypes, 37/51 residual errors stereotype-aligned). For Race/Ethnicity, the zero-shot baseline of 0.333 reflects that the base model was already \emph{counter-stereotypical} in this category; Fair-RLVR's shift to 0.500 represents the model becoming neutral rather than counter-stereotypical, which registers as a worsened ratio. The $n$\,ambig column clarifies that Race/Ethnicity has only 6 total errors ($n$\,ambig=180), so this ratio is extremely noisy. Expressed as Absolute Stereotype Error, ASE decreases across all nine categories, confirming the fairness reward reduces total stereotype harm even when the error ratio worsens.
...
All values of $\lambda$ substantially outperform the zero-shot baseline in training-batch statistics (66.1\% zero-shot ambig accuracy, 0.717 stereotype rate). \textbf{Held-out evaluation was performed only for the main $\lambda=0.5$ run} (Table~\ref{tab:main}); the remaining rows reflect on-policy batch estimates from the training distribution, which are optimistically biased because the model is actively optimizing that distribution at the time of measurement. Batch stereotype rates near zero during training do not imply the same on held-out evaluation. The key trend is at $\lambda = 1.0$: batch accuracy drops slightly (92.6\%) and the stereotype rate rises (0.065) compared to mid-range $\lambda$ values, consistent with the prediction that very high $\lambda$ induces excessive abstention. Values of $\lambda \in [0.3, 0.7]$ perform similarly, indicating Fair-RLVR is not highly sensitive to the exact choice of fairness weight within this range. Confirming this trend with held-out evaluation across all $\lambda$ values remains future work.
